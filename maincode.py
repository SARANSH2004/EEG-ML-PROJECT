    ...
    for subject in subjects:
        ...
        for channel in numeric_cols:
            ...
            for band_name, band_freq in frequency_bands.items():
                band_powers[band_name] = compute_bandpower_welch(
                    channel_data, band_freq, fs)
            ...
            threshold = 0.15
            dominant_bands = [
                band for band, power in normalized_powers.items() if power >= threshold]
        ...
        subject_results.append(pd.DataFrame(
            subject_channel_results, columns=frequency_bands.keys()))
...
def analyze_single_channel(uploaded_file):
    ...
    st.write(
        f"**Recommendations based on {dominant_band} state:** {recommendation}")
...
def plot_3d_brain(brain_regions, subject_dominant_bands, selected_subject, mental_states):
    ...
    for channel, info in brain_regions.items():
        x, y, z = info["coords"]
        dominant_band = subject_dominant_bands.get(
            selected_subject, {}).get(channel, None)
...
def plot_affected_power_distribution(subject_dominant_bands, df, selected_subject, numeric_cols):
    if selected_subject not in subject_dominant_bands:
        st.error(
            f"No dominant band information available for Subject {selected_subject}.")
        return

    subject_data = df[df['Subject'] == selected_subject]
    channel_band_powers = subject_dominant_bands[selected_subject]
    st.write(
        f"### Affected Channel-wise Power Distribution for Subject {selected_subject}")

    for channel in numeric_cols:
        if channel in channel_band_powers:
            st.write(
                f"**Channel:** {channel} - **Dominant Band:** {channel_band_powers[channel]}")
...
def main():
    ...
    analysis_option = st.selectbox("Select Analysis Type", [
                                   "Multi-Channel Analysis", "Real-Time Single Channel Analysis"])

    if analysis_option == "Multi-Channel Analysis":
        uploaded_file = st.file_uploader(
            "Upload Multi-Channel CSV", type=["csv"])
        if uploaded_file:
            df, numeric_cols, subjects = load_and_preprocess_data(
                uploaded_file)
            st.write("### Model Training and Evaluation")
            report_df, subject_dominant_bands = train_model(
                df, numeric_cols, subjects, get_brain_regions())

            selected_subject = st.selectbox(
                "Select Subject for Visualization", subjects)

            # Moved report generation to occur only after selecting a subject
            if selected_subject:
                st.write("### Model Classification Report for Selected Subject")
                selected_subject_band_info = subject_dominant_bands.get(
                    selected_subject, {})
                subject_band_df = pd.DataFrame(selected_subject_band_info.items(), columns=[
                                               "Channel", "Dominant Band"])
                st.dataframe(subject_band_df)
            ...
            tab1, tab2, tab3 = st.tabs(
                ["Recommendations", "3D Brain Visualization", "Channel-wise Power Distribution"])

            with tab1:
                dominant_bands = subject_dominant_bands[selected_subject]
                recommendations = {band: generate_recommendations(
                    band) for band in dominant_bands.values()}
            ...
            with tab2:
                ...
                plot_3d_brain(brain_regions, subject_dominant_bands,
                              selected_subject, {})

            with tab3:
                st.write("### Channel-wise Power Distribution")
                plot_affected_power_distribution(
                    subject_dominant_bands, df, selected_subject, numeric_cols)

    elif analysis_option == "Real-Time Single Channel Analysis":
        uploaded_file = st.file_uploader(
            "Upload Single Channel CSV", type=["csv"])
        if uploaded_file:
